        ...
        <template v-if="schema.type === 'object'">
          ...
          >
            <div v-if="property.type !== 'object' && property.type !== 'array'" class="field-container">
              <label :for="`${fieldPrefix}${key}`">{{ property.title || key }}{{ schema.required?.includes(key) ? ' *' : '' }}</label>
              
              <div v-if="property.description" class="field-description">{{ property.description }}</div>
              
              <!-- String type -->
              <input 
                v-if="property.type === 'string' && !property.enum && !property.format"
                :id="`${fieldPrefix}${key}`"
              ...
              <!-- Email format -->
              <input 
                v-else-if="property.format === 'email'"
                :id="`${fieldPrefix}${key}`"
                v-model="formData[key]"
                type="email"
                :required="schema.required?.includes(key)"
            ...
            <div v-else-if="property.type === 'object'" class="nested-object">
              ...
              <JsonSchemaForm
                ...
                @update:modelValue="updateNestedValue(key, $event)"
                :errors="getNestedErrors(key)"
                :field-prefix="`${fieldPrefix}${key}.`"
                :level="level + 1"
              />
            </div>
            ...
            <div v-else-if="property.type === 'array'" class="array-field">
              ...
              <div v-if="['string', 'number', 'integer', 'boolean'].includes(property.items.type)" class="array-primitives">
                <div 
                  v-for="(_, index) in formData[key] || []"
                  :key="`${key}_${index}`"
                  class="array-item"
                >
                  <!-- String item -->
                  <input 
                    v-if="property.items.type === 'string' && !property.items.enum && !property.items.format"
                  ...
                  <!-- Number item -->
                  <input 
                    v-else-if="property.items.type === 'number' || property.items.type === 'integer'"
                  ...
                  <!-- Boolean item -->
                  <div v-else-if="property.items.type === 'boolean'" class="checkbox-wrapper">
                    <input 
                      v-model="formData[key][index]"
                      type="checkbox"
                    />
                ...
                <button 
                  type="button" 
                  class="array-add-btn"
                  @click="addArrayItem(key, property.items.type)"
                >
                  Add Item
                </button>
              </div>
              ...
              <!-- Array of objects -->
              <div v-else-if="property.items.type === 'object'" class="array-objects">
                <div 
                ...
                >
                  ...
                  <JsonSchemaForm
                    ...
                    :field-prefix="`${fieldPrefix}${key}[${index}].`"
                    :level="level + 1"
                  />
                </div>
                ...
                <button 
                  type="button" 
                  class="array-add-btn"
                  @click="addArrayObjectItem(key, property.items)"
                >
                  Add Item
                </button>
              </div>
